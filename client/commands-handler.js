...
export function executeCommand(command) {
  ...
  if (parsed.length == 0) {
    socket.emit("log", ERROR_MSG.UNEXISTING_COMMAND);
    return;
  }
  ...
  try {
    ...
    if (err == ERROR.INVALID_ADDRESS) {
      socket.emit("log", ERROR_MSG.INVALID_ADDRESS);
    }
...
function registerAttack(attack) {
  attack["timeout"] = setTimeout(() => {
    ...
    socket.emit("log", "attack of id: " + attack.id + " timed out");
  }, ATTACK_TIMEOUT_MS);
...
function removeAttack(id) {
  ...
  attacks.delete(id);
  socket.emit("log", "attack of id: " + id + " completed");
}
...
function getCurrentAttacks() {
  attacks.forEach((attack) => {
    socket.emit("log", "ID: " + attack.id + " Type: " + attack.type + " Address: " + attack.address);
  });
}
